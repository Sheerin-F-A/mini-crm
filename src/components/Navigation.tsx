...
export function Navigation() {
  ...
  return (
    <div className="w-64 bg-[#f4f0eb] border-r-2 border-black min-h-screen flex flex-col z-10 relative">
      <div className="retro-window-header border-b-2 border-black">
        <span>XENO.EXE</span>
          ...
          return (
            <Link
              ...
              className={`flex items-center space-x-3 px-4 py-3 border-2 border-transparent transition-all ${
                isActive 
                  ? 'retro-box bg-[#ffcf54] text-black border-black' 
                  : 'text-slate-700 hover:border-black hover:bg-slate-200'
              }`}
            >
              <Icon size={20} strokeWidth={isActive ? 2.5 : 2} className="text-black" />
      ...
      <div className="p-4 m-4 retro-box bg-white">
        ...
        <p className="mt-2 text-xs text-slate-500 font-mono">v1.0-RETRO</p>
      </div>
    </div>
  );
}
